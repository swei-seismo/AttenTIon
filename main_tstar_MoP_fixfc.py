...
## MAIN PROGRAM TO INVERT t* FOR ATTENUATION TOMOGRAPHY WITH FIXED CORNER FREQUENCY
...
##subdb='4'
##workdir='/P/weisq/attentomo/tstarnnls_10MPa0.27'
##alpha=[0.27]
##dstress=10.0
...
##            if saving[sta][2]['misfit'][1]<misfitS:
            if saving[sta][2]['fitting'][1]>misfitS:
                staS3lst.append(sta)
            k1=k2
    ###### 3 MEANS INVERTING AGAIN WITHOUT MOELD ERROR > 0.03 ######
        if len(staS3lst)<2:
            print('Not enough good S wave record for event %d.' % orid)
            continue
...
##            saving[sta][3]['tstar'].append(tstarS[ista][0])
    ...
    ###### 3 MEANS INVERTING AGAIN WITHOUT misfit>1 ######
        ...
        for icnts in range(len(staS3lst)):
            sta=staS3lst[icnts]
            ftstar.write('%s  %.4f  %.4f  %.4f  %f  %f  %.2f  %f  %f  %.2f  %.2f\n' %
                         (sta,
                          ORIG['lat'],
                          ORIG['lon'],
                          ORIG['dep'],
