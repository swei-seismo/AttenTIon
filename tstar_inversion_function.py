...
def Loop_Record(orid, stalst, param, ARRIV, PGS, SGS):
    ...
    for i, sta in enumerate(stalst):
        ...
        (p_dd,pn_dd) = tstarsub.fixwin(dd,tt,param,ARRIV[sta],orid)
        ...
        (goodP2, goodS2, spec_px, freq_px, pspec, pn_spec, pfreq, pn_freq, frmin, frmax) \
            = tstarsub.dospec(PWINDATA, orid, param, 2, ARRIV[sta])
        if not goodP2:
        ...
        if param['source_para'] == 1:   ## search for fc
            ...
            (goodP1, goodS1, spec_px, freq_px, pspec, pn_spec, pfreq, pn_freq, frmin, frmax) \
                 = tstarsub.dospec(PWINDATA, orid, param, 1, ARRIV[sta])
            if not goodP1:
...
def inversion(orid, saving, stalst, ORIG, POS, icase, param):
    ...
    data = tstarsub.buildd(saving,stalst,ORIG,POS,idata,param,ORIG['fc'])
    G = tstarsub.buildG(saving,stalst,param['alpha'],POS,idata,param)
...
def bestfc(orid, saving, stalst, ORIG, POS, icase, param):
    ...
    for ifc in range(len(fc)):
        data = tstarsub.buildd(saving,stalst,ORIG,POS,icase,param,fc[ifc])
        model,residu = nnls(G[:,:,0],data[:,0])
        lnmomen = model[0]      ## MOMENT
        tstar = model[1:]       ## t*
...
def output_results(orid, staP3lst, param, ORIG, saving, ARRIV):
    ...
    if param['doplotspec']:
        for sta in staP3lst:
            if saving[sta][2]['good'][0]:
                ...
                tstarsub.plotspec(saving[sta],param,ARRIV[sta],orid, \
                                  'P',lnM,ORIG['fc'],param['alpha'],3)
