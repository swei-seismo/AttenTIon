...
snrcrts2=[1.2,1.0]    ## MINIMUM [SNR,FREQUENCY BAND WIDTH] FOR t* INVERSION
...
subdb='4'
workdir='/P/weisq/attentomo/tstar_MoS1.2_fcp0.0'
alpha=[0.0]
...
###### 3 MEANS INVERTING AGAIN WITHOUT misfit>1 ######
    ...
    Mwp=float(2.0/3.0*np.log10(momenP*1e7)-10.73)
    logfl.write('Mw(P) = %.2f, Mwisc = %.2f\n' % (Mwp,Mwisc))
...
##        lnmomenS=lnmomenP
        ...
        GS2=tstarsub.buildGmos(saving,staS2lst,alpha,'S',2)
        ialco=alpha.index(bestalpha)
...
##            if saving[sta][2]['fitting'][1]<misfitS and saving[sta][2]['misfit'][1]<1 and saving[sta][2]['err'][1]<1:
    ...
    ###### 3 MEANS INVERTING AGAIN WITHOUT misfit>1 ######
        if len(staS3lst)<2:
            print('Not enough good S wave record for event %d.' % orid)
            continue
        dataS3=tstarsub.buildd(saving,staS3lst,bestfcs,'S',3)
        GS3=tstarsub.buildGmos(saving,staS3lst,alpha,'S',3)
        ialco=alpha.index(bestalpha)
...
##        modelS,residuS,mrank,sv=lstsq(GS3[:,:,ialco],dataS3)
        ...
        lnmomenS=modelS[0]      ## MOMENT
        tstarS=modelS[1:]
        momenS=np.exp(lnmomenS)
        Mws=float(2.0/3.0*np.log10(momenS*1e7)-10.73)
        logfl.write('Mw(S) = %.2f, Mwisc = %.2f\n' % (Mws,Mwisc))
        print(lnmomenP,lnmomenS,Mwp,Mws)
        ## SAVE RESIDUL AND MISFIT OVER ALL
        allSres=allSres+(residuS**2)/dataS3.shape[0]
        allSmisfit=allSmisfit+residuS/dataS3.shape[0]
        alltS=alltS+1
        ## ESTIMATE t* ERRORS BASED ON DATA VARIANCES FOR EACH t*
