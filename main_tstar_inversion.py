...
for ior in np.arange(bgn,bgn+NUM,1):
    ...
    stalst, ARRIV  = tstar_load.loaddata(param, orid, tp.sacdir+"/%s" %(orid))
    if len(stalst) == 0:
    ...
    ## 3rd inversion: IVERTING AGAIN WITHOUT fitting < 0.85
    (ORIG, saving) = tf.inversion(orid, saving, staP3lst, ORIG, 'P', 3, param)

    ## Output the results
    tf.output_results(orid, staP3lst, param, ORIG, saving, ARRIV)
