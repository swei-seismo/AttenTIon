...
def loadORIG(orid, param):
    ...
    if param['source_para'] == 1:
        ORIG['mb'] = float(event_info.split()[4])
    
    elif param['source_para'] == 2:
        SSlst = "/mnt/home/zhuoran/sac_test/data/Alaska_events_source_spectra.dat"
...
def loaddata(param, orid, fldir):
    ...
    for sacfl in sacfls:
        ...
        cmd = "saclst o t0 t1 dist baz gcarc delta f %s" %(sacfl)
        output = os.popen(cmd).read().strip()
        otime = output.split()[1]
        T0 = output.split()[2]      
        T1 = output.split()[3]    
        dist = output.split()[4]
        baz = output.split()[5]
        gcarc = output.split()[6]
        dt = output.split()[7]
        if T0 == '-12345':
            T0 = -1
        if T1 == '-12345':
        ...
        ARRIV.setdefault(sta,{})['otime'] = float(otime)
...
def loadsite(load_site):
    """
    Invert for t* for the second time with correcting site effects
    """
    ...
    allPsite = {}
    sitelst = glob.glob("data/sitespec/*_Psitespec.dat")
    for i in range(len(sitelst)):
        sitefl = sitelst[i]
        sta = os.path.basename(sitefl)
        sta = sta.split("_")[0]
        sitedata = np.loadtxt(sitefl)
        sitedata = np.nan_to_num(sitedata)
        allPsite[sta] = sitedata
    
    allSsite = {}
    sitelst = glob.glob(tp.sitedir+"/"+"*_Ssitespec.dat")
    for sitefl in range(len(sitelst)):
        filename = os.path.basename(sitefl)
        staname = filename.split("_")[0]
        sitedata = np.loadtxt(sitefl)
        sitedata = np.nan_to_num(sitedata)
        allSsite[staname] = sitedata
    
    site = [allPsite, allSsite]
