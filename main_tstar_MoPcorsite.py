...
## MAIN PROGRAM TO INVERT t* FOR ATTENUATION TOMOGRAPHY AFTER SITE EFFECT CORRECTION
## Written by S. Wei, Nov. 2013
## Modified by S. Wei for site effect correction, May. 2018
...
########## READ SITE EFFECTS ############
sitedir='/mnt/home/swei/LauQtomo/sitespec'
allPsite={}
sitefllst=glob.glob(sitedir+'/Psite*.spec')
...
allSsite={}
sitefllst=glob.glob(sitedir+'/Ssite*.spec')
for isite in range(len(sitefllst)):
    sitefl=sitefllst[isite]
    stn=os.path.basename(sitefl)
    stn=stn.split('.')[0][6:]
...
##            print('Deleting %s' % delvar)
            del globals()[delvar]
    logfl.write('\nWorking on ORID # %d\n' % orid)
    print('============Working on ORID # %s_%d============' % (subdb,orid))
...
##        exit()
    ...
    ## BUILD d MATRIX WITH VARIABLE CORNER FREQUENCY TO FIND BEST fc AND alpha
            dataP1=tstarsub.buildd_site(saving,staP1lst,fc[ifc],allPsite,'P',1)
...
##            tsfc[ifc,:]=tstarP[:,ialco]
    ...
    dataP2=tstarsub.buildd_site(saving,staP2lst,bestfcp,allPsite,'P',2)
...
###### 3 MEANS INVERTING AGAIN WITHOUT fitting < 0.85 ######
    if len(staP3lst)<5:
        print('Not enough good P wave record for event %d.' % orid)
        continue        
    dataP3=tstarsub.buildd_site(saving,staP3lst,bestfcp,allPsite,'P',3)
...
###### 3 MEANS INVERTING AGAIN WITHOUT misfit>1 ######
        ...
        for sta in staP3lst:
            if saving[sta][2]['good'][0]:
                ...
                if sta in allPsite.keys():
                    sitedata=allPsite[sta][:,1]
                else:
                    sitedata=0
                tstarsub.plotspec(saving[sta],sta,orid,'P',lnmomenP,
                                  bestfcp,bestalpha,3,sitedata)
    ...
    if len(staS2lst)<2:
        ...
        dataS2=tstarsub.buildd_site(saving,staS2lst,bestfcs,allSsite,'S',2,lnmomenS)
...
##            if saving[sta][2]['fitting'][1]<misfitS and saving[sta][2]['misfit'][1]<1 and saving[sta][2]['err'][1]<1:
    ...
    ###### 3 MEANS INVERTING AGAIN WITHOUT misfit>1 ######
        if len(staS3lst)<2:
            print('Not enough good S wave record for event %d.' % orid)
            continue
        dataS3=tstarsub.buildd_site(saving,staS3lst,bestfcs,allSsite,'S',3,lnmomenS)
