...
## MAIN PROGRAM TO INVERT t*(P) FOR EACH EVENT AND t*(S) FOR EACH EVENT, STATION
...
hico=9.9
npol=4
snrcrtp1=[3.0,4.]    ## MINIMUM [SNR,FREQUENCY BAND WIDTH] FOR FINDING BEST fc AND alpha
...
snrcrts2=[2.0,1.0]    ## MINIMUM [SNR,FREQUENCY BAND WIDTH] FOR t* INVERSION
...
##doplotspec=bool(s.split()[3])
####print(doplostpec)

subdb='4'
workdir='/P/weisq/attentomo/tstar_indS2.0_fcp0.0'
alpha=[0.0]
...
##    lnmomenS=lnmomenP
    ...
    for ista in range(len(staS2lst)):
        sta=staS2lst[ista]
        freq_x = saving[sta][2]['s'][0]
        spec_x = saving[sta][2]['s'][1]
        correc = saving[sta]['corr'][1]
        dataS=(np.log(spec_x)-np.log(correc)
                        +np.log(1+(freq_x/bestfcs)**2)).transpose()
        GS=np.array([-np.pi*freq_x**(1-bestalpha)]).transpose()
        GS=np.hstack((np.ones((dataS.shape[0],1)),GS))
##        print(GS.shape,dataS.shape)
##        modelS,residuS,mrank,sv=lstsq(GS,dataS)
        modelS,residuS=nnls(GS,dataS)
        GSinv=np.linalg.inv(np.dot(GS.transpose(),GS))
        syns=np.dot(GS,modelS)
        sterr=np.sqrt(np.linalg.norm(dataS-syns)**2/(dataS.shape[0]-2)
                     *GSinv.diagonal()[1])  
        ststar=[float(modelS[1]),sterr,float(residuS**2/dataS.shape[0]),
                float(residuS/dataS.shape[0])]
        saving[sta]['lnMs']=modelS[0]
##        dtstar=ststar[0]-saving[sta][3]['tstar'][0]
        allSres=allSres+ststar[2]
        allSmisfit=allSmisfit+ststar[3]
        alltS=alltS+1
        try:
            saving[sta][3]
        except KeyError:
            ...
            saving[sta][3]['aveATTEN']=[0]
            saving[sta][2]['fitting']=[1000]
        saving[sta][3]['tstar'].append(ststar[0])
        saving[sta][3]['misfit'].append(ststar[1])
        saving[sta][3]['err'].append(ststar[1]) ## cov(m)=cov(d)inv(G'G) FOR OVERDETERMINED PROBLEM
        saving[sta][3]['aveATTEN'].append((1000*ststar[0]/saving[sta]['Stt']))
        if saving[sta][3]['aveATTEN'][0]==0:
            saving[sta][3]['QpQs']=1.75
        else:
            saving[sta][3]['QpQs']=saving[sta][3]['aveATTEN'][1]/saving[sta][3]['aveATTEN'][0]
            
##        ftstar.write('%s  %.4f  %.4f  %.4f  %f  %f  %f  %f  %.2f\n' % (sta,
##                ORIG['lat'],ORIG['lon'],ORIG['dep'],ststar[0],sterr,dtstar,
##                                                             satten,QpQs))
        ftstar.write('%s  %.4f  %.4f  %.4f  %f  %f  %.2f  %.2f  %.2f\n' %
                     (sta,ORIG['lat'],ORIG['lon'],ORIG['dep'],
                      saving[sta][3]['tstar'][1],saving[sta][3]['err'][1],
                      saving[sta][3]['aveATTEN'][0],saving[sta][3]['aveATTEN'][1],
                      saving[sta][3]['QpQs']))
    ftstar.close()
    ...
    if doplotspec:
        for sta in staS2lst:
            if saving[sta][2]['good'][1]:
                ...
                tstarsub.plotspec(saving[sta],sta,orid,'S',saving[sta]['lnMs'],
                                      bestfcs,bestalpha,3)
